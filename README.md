...
## Optional: AI summaries with verbatim quotes

Each catalog card supports a TL;DR + verbatim-quotes + transcript view, generated by `summarize.py`. The cards auto-upgrade when `summaries.json` is present.

```bash
brew install poppler tesseract                  # text extraction + OCR
pip install anthropic
export ANTHROPIC_API_KEY=sk-ant-...
python3 summarize.py                            # ~30–90 min, ~$10–25 in API cost
```
